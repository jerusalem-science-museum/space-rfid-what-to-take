...
class VideoPlayer:
    ...
    Input is read from stdin — the RFID reader acts as a USB HID keyboard,
    so scans appear as typed lines in the terminal (tmux session).
    """
    ...
    def add_new_pucks(self):
        ...
        while add_pucks:
            ...
            other_side = input("set other side? Y/n: ").strip().lower()
            if other_side == 'y' or other_side == '':
                self._add_new_puck(path)
            another_puck = input("set another puck? Y/n: ").strip().lower()
            if another_puck == 'n':
                add_pucks = False
